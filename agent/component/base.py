...
class ComponentParamBase(ABC):
    def __init__(self):
        ...
        self.inputs = []
        self.debug_inputs = []
        # 初始化output和infor属性，避免AttributeError
        setattr(self, self.output_var_name, None)
        setattr(self, self.infor_var_name, None)
...
class ComponentBase(ABC):
    ...
    def __str__(self):
        ...
        """
        # 安全地获取output属性，如果不存在则使用None
        out = getattr(self._param, self._param.output_var_name, None)
    ...
    def output(self, allow_partial=True) -> Tuple[str, Union[pd.DataFrame, partial]]:
        # 安全地获取output属性，如果不存在则使用None
        o = getattr(self._param, self._param.output_var_name, None)
